...
- `skip_mount_device` (bool) - Optionally skip mounting the device.  Default false.  If true, 
  the pre_mount_commands, mount device step, post_mount_commands, and mount extra steps are skipped.  
  A custom provisioner must be used to manually configure and mount the device
